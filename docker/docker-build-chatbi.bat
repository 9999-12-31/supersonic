...
docker build --no-cache --build-arg SUPERSONIC_VERSION=%version% -t harbor.bigdata.com/chatbi/supersonic:%version% -f docker\Dockerfile-chatbi .
if %errorlevel% neq 0 (
    echo Docker build failed. Exiting.
    exit /b 1
)
echo Docker image supersonic:%version% built successfully.
docker push harbor.bigdata.com/chatbi/supersonic:%version%
goto :eof
